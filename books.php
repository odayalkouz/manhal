                                                        ...
                                                        <div id="year" class="wrapper-dropdown-3" tabindex="5">
                                                            ...
                                                            <ul class="dropdown scrollable" style="max-height: 186px">
                                                                ...
                                                                foreach ( range( $latest_year, $earliest_year ) as $i ) {
                                                                    echo ' <li class="catli" catid="'.$i.'"><a href="#"><i class="icon-truck icon-large">'.$i.'</i></a> </li>';
                                                                }
                                                                ?>
                                                            </ul>
                                                        </div>
                        ...
                        <div class="right-col-3 floating-left">
                            ...
                            if(isset($_GET["book_type"]) && trim($_GET["book_type"])!=""){
                                switch($_GET["book_type"]){
                                    case "electronic":
                                        $type_filter = " AND `books`.`booktype`in(2,3,6,7)";
                                        break;
                                    case "interactive":
                                        $type_filter = " AND `books`.`booktype`in(4,5,6,7)";
                                        break;
                                    case "paper";
                                        $type_filter = " AND `books`.`booktype`in(1,3,5,7)";
                                        break;
                                    default :
                                        $type_filter = " AND `books`.`booktype`in(1,3,5,7)";
                                        break;
                          ...
                          /*  $price_filter="";
                            ...
                            if (isset($_GET['keywords']) && $_GET['keywords'] != "") {
                                $keyword_filter = " AND ( `name` LIKE '%" . mysqli_real_escape_string($con, $_GET['keywords']) . "%' OR `isbn` LIKE '%" . mysqli_real_escape_string($con, $_GET['keywords']) . "%' )";
                            }
                            $cat_filter = "";
                            ...
                            $sql = "SELECT `books`.*,`categories`.*,`books`.`rating_count` as rate_count FROM `books` left OUTER JOIN `categories` ON `books`.`category`=`categories`.`catid` WHERE `books`.`status` =1 AND `books`.`store`=0 AND( `books`.`status`=1 " . $keyword_filter . $cat_filter.$grade_filter.$age_filter.$year_filter.$isbn_filter.$lang_filter.$price_filter.$semester_filter.$booktype_filter.$type_filter." ) ORDER BY `books`.`bookid` ,`books`.`name` DESC ,`books`.`category`, `books`.`_group` DESC ";
...
//                            }
                            ...
                            $sql = "SELECT `books`.*,`categories`.*,`books`.`rating_count` as rate_count FROM `books` left OUTER JOIN `categories` ON `books`.`category`=`categories`.`catid` WHERE `books`.`status` =1 AND `books`.`store`=0 AND( `books`.`status`=1 " . $keyword_filter . $cat_filter .$grade_filter.$age_filter.$year_filter.$isbn_filter.$lang_filter.$price_filter.$semester_filter.$booktype_filter.$type_filter." ) "." ORDER BY  `books`.`name` DESC ,`books`.`category`, `books`.`_group` DESC".$pagination[0];
                            $result = $con->query($sql);
                            ?>
                            <div class="books-main-container-page clear-both">
                                ...
                                <div class="ui-panels">
                                    ...
                                    if(((isset($_GET['page']) && $_GET['page']<2) || !isset($_GET['page']) || $_GET['page']=='') && !isset($_GET['search'])){
                                        include_once ("books_campain.php");
                                    }



                                    while ($books = mysqli_fetch_assoc($result)) {
                                        echo PaintBook($books,'StudentBook');
                                    }
                                    ?>
                                </div>
                            </div>
