                        ...
                        <div class="right-col-3 floating-left">
                            <div class="books-main-container-page clear-both">
                                ...
                                <div class="ui-panels-worksheet">
                                    ...
                                    $sql = "Select media.*,categories.name_ar, categories.name_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type=0 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter." ORDER BY `media`.`price` ASC, `media`.`is_playlist` DESC, `media`.`id` DESC ";
...
//                                    }
                                    ...
                                    $sql = "Select media.*,categories.name_ar, categories.name_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type=0 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter." ORDER BY `media`.`price` ASC, `media`.`is_playlist` DESC, `media`.`id` DESC " . $pagination[0];


                                    $result = $con->query($sql);
                                    while ($game = mysqli_fetch_assoc($result)) {
                                        echo paintmedia($game, 'worksheet');
                                    }
                                    ?>
                                    <div class="paging">
                                        <div class="content">
