                            ...
                            <div class="books-main-container-page clear-both">
                                <div class="top-black-col">
                                    <div class="floating-left">
                                        <label class="lbl-data-a floating-left stories"><?= $Lang->Category; ?></label>
                                        <div class="wrapper-demo">
                                            <div id="subject" class="jq_bookdropdown wrapper-dropdown-3" tabindex="2">
                                                <span id="category_span"
                                                      style="background-image: url(<?= SITE_URL; ?>themes/main-Light-green-En/images/cat/books/all.svg);"
                                                      class="floating-left"><?= $Lang->All; ?></span>
                                                <ul id="category_ul" class="dropdown submit scrollable">
                                                ...
                                                <script>
                                                    ...
                                                    $("#category_span").css("background-image", $("#category_adv_ul li.active").find("a").first().css("background-image"));
                                                    $(".jq_bookdropdown span").css("background-image", $(this).children("a").css("background-image"));
                                                </script>
                                            </div>
                                        </div>
                                ...
                                <div class="ui-panels-worksheet">
                                    ...
                                    if (isset($_GET["search"]) && $_GET["search"] == "advanced") {
                                        ...
                                        if (isset($_GET["language_arabic"]) && $_GET["language_arabic"] == '1') {
                                            $lang_filter2 .= "`playlist`.`language`='Ar'";
                                        }
                                        if (isset($_GET["language_english"]) && $_GET["language_english"] == '1') {
                                            if ($lang_filter2 == "") {
                                                $lang_filter2 = "`playlist`.`language`='En'";
                                            } else {
                                                $lang_filter2 .= " OR `playlist`.`language`='En'";
                                            }
                                        }
                                        if (isset($_GET["language_french"]) && $_GET["language_french"] == '1') {
                                            if ($lang_filter2 == "") {
                                                $lang_filter2 .= "`playlist`.`language`='Fr'";
                                            } else {
                                                $lang_filter2 .= " OR `playlist`.`language`='Fr'";
                                            }
                                        }
                                        ...
                                        if (isset($_GET["search_age"]) && $_GET["search_age"] > 0) {
                                            $age_filter = " AND `playlist`.`age`=" . $_GET["search_age"];
                                        }

                                        if (isset($_GET["search_grade"]) && $_GET["search_grade"] > 0) {
                                            $grade_filter = " AND `playlist`.`grade`=" . $_GET["search_grade"];
                                        }
                                    }
                                    ...
                                    $sql = "Select playlist.*,categories.name_ar, categories.name_en From playlist Inner Join categories On playlist.category = categories.catid Where playlist.status = 1 And playlist.id > 0 And playlist.type=30 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter . " ORDER BY `playlist`.`id` DESC";
                                    ...
                                    $sql = "Select playlist.*,categories.name_ar, categories.name_en From playlist Inner Join categories On playlist.category = categories.catid Where playlist.status = 1 And playlist.id > 0 And playlist.type=30 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter . " ORDER BY `playlist`.`price` ASC, `playlist`.`id` DESC " . $pagination[0];
                                    $result = $con->query($sql);
                                    while ($playlist = mysqli_fetch_assoc($result)) {
                                        echo paintmedia($playlist, 'playlist');
                                    }
                                    ?>
                                    <div class="paging">
                                        <div class="content">
