...
mouseMove = function (e) {
    ...
    if(eval(e.target.widget).activ==false){return}
...
mouse_up = function (e) {
    ...
    if(eval(elem.widget)==undefined||eval(elem.widget).activ==false){return}
...
MatchingOOP.prototype.redrawLines = function () {
    ...
    for (var i = 0; i < this.linesArray.length; i++) {
        ...
        if (this.linesArray[i] == "undefined × 1" || this.linesArray[i].indexStartBox=='' ) {
        } else {
...
function checkAnswer(object) {
    ...
    for (var i = 0; i < object.linesArray.length; i++) {
        ...
        if (startPoint == endPoint) {
           // $('<img class="imageCheckAnswer" src="img/checkright.png">').appendTo('#' + object.linesArray[i].indexStartBox)
            count++
        }
        else {
           /// $('<img class="imageCheckAnswer" src="img/wrong.png">').appendTo('#' + object.linesArray[i].indexStartBox)
        }
        $("#" + object.linesArray[i].indexStartBox).attr('checkedValue', 'yes');
    }

    addXToAllNoneAnswerd();
    return {count:count,total:object.linesArray.length};
    alert("Score : " + count + "/" + object.linesArray.length)
}
