...
class store
{
    ...
    public function LoginUserManhal($name, $pass)
    {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $data = array("userid" => $row['userid'], "uname" => $row['uname'], "fullname" => $row['fullname'], "email" => $row['email'], "avatar" => $this->URL . $row['avatar'], "address" => $row['address'], "gender" => $row['gender'], "birthdate" => $row['birthdate'], "permession" => $row['permession'], "token" => $token);
 ...
 IF(`payments_books`.`itemtype`='book',IF(`books`.status=1,'https://www.manhal.com/platform/books/###/cover.jpg',''),IF(`payments_books`.`itemtype`='story',IF(`story`.storyid>0   ,'https://www.manhal.com/platform/stories/$$$/story/###/images/pic.jpg',''),IF(`products`.productid>0   ,'https://www.manhal.com/platform/products/###/thumbnail_small.jpg',''))) as `thumbnail`
    ...
    {
        $sql = "SELECT * FROM `storeslider` WHERE `id`>0 and _delete=0 ORDER BY `storeslider`.`sort` ASC";
        $result = $this->conn->query($sql);
        if (mysqli_num_rows($result) > 0) {
            $data = [];
            $i = 0;
    ...
    public function GetDeliveryCost($city)
    {
        ...
        $price = $this->convertCurrency($cost);
        $data = array("cost" => $price, "city" => $city);
        $result = $this->msg1($this->msg16_txt(), $data, 0);
        return $result;
    }
    ...
    public function savePaymentToDB($products_price, $idlocation, $items, $additional_info)
    {
        $transaction = 'Manhal_cash';
        $RecieverCompanyName='';
        $RecieverAttention='';
        $Address1='';
        $Address2='';
        ...
        if ($user != 'false') {
            ...
            if ($location == 'false') {
                ...
                $RecieverCompanyName=$location['name'];
                $RecieverAttention=$location['name'];
                $billing_fullname==$location['name'];
                $Address1=$location['address'];
                $billing_address1=$location['address'];
                $Address2=$location['building']."----".$location['apr']."----".$location['floor'];
                $billing_address2=$location['building']."----".$location['apr']."----".$location['floor'];
                $City=$location['region'];
                $StateProvince=$location['region'];
                $billing_country=$location['region'];
                $Country=$location['region'];
                $billing_city=$location['region'];
                $billing_state=$location['region'];
                $billing_zipcode=$location['region'];
                $Phone=$location['mob'];
                $telephone=$location['mob'];
                $billing_mobile=$location['mob'];
                $billing_telephone=$location['mob'];
            }
            $shippingprice = $this->DeliveryCost($location['region']);
            $shippingC = "Manhal";
            $status = 0;
            $payment_type = 'cash';
            $contents = 'all';
            $Countrycode = 'JO';
            $total_price = $products_price + $shippingprice;
            $DeclaredValue= $products_price + $shippingprice;
            $tax = 0;
            $cod = 0;
            $manhal_ref="subscribe_".uniqid();
 ...
 ,'".$manhal_ref."'
 ,'" . $payment_type . "'
 ,'" . mysqli_real_escape_string($this->conn, $RecieverCompanyName) . "'
 ,'" . mysqli_real_escape_string($this->conn, $RecieverAttention) . "'
 ,'" . mysqli_real_escape_string($this->conn, $Address1) . "'
 ,'" . mysqli_real_escape_string($this->conn, $Address2) . "'
 ,'" . mysqli_real_escape_string($this->conn, $City) . "'
 ,'" . mysqli_real_escape_string($this->conn, $StateProvince) . "'
 ,'" . mysqli_real_escape_string($this->conn, $Postcode) . "'
 ,'" . mysqli_real_escape_string($this->conn, $Country) . "'
 , " . $weight . "
 ,'" . mysqli_real_escape_string($this->conn, $Phone) . "'
 ,'" . mysqli_real_escape_string($this->conn, $contents) . "'
 ,'" . $DeclaredValue . "'
 ,".$exported."
 ,'" . $shippingprice . "'
 ,'" . $Countrycode . "'
 ,'" . mysqli_real_escape_string($this->conn, $telephone) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_fullname) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_email) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_mobile) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_telephone) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_country) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_city) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_state) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_zipcode) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_address1) . "'
 ,'" . mysqli_real_escape_string($this->conn, $billing_address2) . "'
 ," . $products_price . "," . $tax . "," . $cod . ")";
            if ($this->conn->query($sql)) {
                ...
                $NoPayment=["id"=>$paymentid];
                $this->SendToNotification($this->getTokenAdmin(),'New purchases of number','New purchases of number',$NoPayment);
                $result = $this->msg1($this->msg13_txt(), $this->json(), 0);
            } else {
    ...
    public function getTokenAdmin(){
        $sql = "SELECT token.token FROM `token` left JOIN users ON users.userid=token.userid WHERE `permession` in (1,7)  ";
        $result = $this->conn->query($sql);
        $data =[];
        if (mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)){
    ...
    public function SendToNotification($to=[] , $title = "", $messege = "", $data = [])
    {
        $url = "https://fcm.googleapis.com/fcm/send";
        $serverKey = "AAAAHxiFlLE:APA91bG7gfsnM_7t4pSLxIJa608Psr-pT7TnTDiGesV0lhJQqxVAV7w2RCxhNtl5a68Psp0gwLfW5op9yKjsk2Pnl_fT_cuTSpiV_OYjYtaIrPXzWIrLQg6HxLdlS8VgXYtPWSoAcEeY";;
        $body = $messege;
        $notification = array('title' =>$title , 'body' => $body, 'sound' => 'default', 'badge' => '1','data'=>$data);
        $arrayToSend = array('registration_ids' =>$to , 'notification' => $notification,'priority'=>'high','data'=>$data);
...
if (isset($_GET["process"]) && $_GET["process"] != "") {
    switch ($_GET["process"]) {
        ...
        case "purchase":
            $k=["id"=>55];
            echo ($api->SendToNotification($api->getTokenAdmin(),'عملية شراء جديدة','يرجى متابعة عملية الشراء رقم :  ',$k));
            break;
