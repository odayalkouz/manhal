...
".$_POST["lms_userid"].",'".mysqli_real_escape_string($con,$_POST["lms_uname"])."','".mysqli_real_escape_string($con,$_POST["lms_password"])."')";
    ...
    public function CheckMedia($type = '',$keyword = "", $category = -1, $from = '', $to = '', $series = 0, $storytype = '', $lang = '', $booktype = '', $id = '', $userid = '', $grade='')
    {
        ...
        switch ($type) {
            ...
            case 'all':
                ...
                $type_filter_Story = " AND `story`.`type` in(2,3,6,7) AND `story`.`status`=1 ";
                ...
                $OR="";
                $sql="SELECT `storyid` as id ,`seriesid`,IF(story.storyid>0,'17','17') as type,IF(story.thumb!='','https://www.manhal.com/platform/stories/$$$/story/###/images/pic.jpg','')as thumbnail,`title` as title_ar ,`title` as title_en,`story`.`catid` as category,`type` as is_playlist,`path` as filename,`path`,`stories_cat`.`name_ar` as category_ar ,`stories_cat`.`name_en` as category_en   FROM `story` left OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid`  WHERE (`story`.`package`=0 AND `story`.`status`=1  AND( `story`.`storyid`>0 " . $this->keyword_Story . $this->category_Story . $this->series_filter_stories_Story . $this->type_Story . $type_filter_Story.$this->grade_Story . " ))  " . $this->FromTo." UNION SELECT `id`,`id` as seriesid,`type`,IF(media.path='','https://www.manhal.com/platform/media/###/thumbnail_small.jpg','https://www.manhal.com/platform/games/###/images/thumb.jpg') as thumbnail,`title_ar`,`title_en`,`category`,`is_playlist`,`filename`,`path`,categories.name_ar as category_ar, categories.name_en as category_en FROM `media` Inner Join categories On media.category = categories.catid  WHERE media.status = 1 And media.id > 0 And media.type" . $typesmedia . $this->keyword_media . $this->category_media.$this->grade_media . " UNION SELECT `quizid` as id ,`quizid` as seriesid,`quizid` as thumbnail,`title` as title_ar,`title` as title_en,`category`, `quizid` as is_playlist,`quizid` as filename, `quizid` as path,`categories`.`name_ar` as category_ar,`categories`.`name_en` as category_en, IF(quiz.quizid>0,'6','6') as `type`  FROM `quiz` Inner Join `categories` On `quiz`.`category` = `categories`.`catid`  WHERE (`quiz`.`is_public`=1".$OR.") ".$keyword_filter_quiz . $this->category_media .  $age_filter_quiz." GROUP BY id order by id DESC  " . $FromTo;
                $result = $this->conn->query($sql);
    ...
    private function getquiz($keyword,$category,$grade,$FromTo){
        ...
        $sql = "SELECT  quiz.*,IF(quiz.quizid>0,'6','6') as `type`,`categories`.`name_ar`,`categories`.`name_en` FROM `quiz` Inner Join `categories` On `quiz`.`category` = `categories`.`catid` WHERE (`quiz`.`is_public`=1".$OR.") ".$keyword_filter . $cat_filter .  $age_filter.  "  " ;
    ...
    private function getmedia($typesmedia, $keyword, $category, $FromTo, $grade)
    {
        ...
        $sql = "Select media.is_playlist,media.type,media.filename,media.id,IF(media.path='','https://www.manhal.com/platform/media/###/thumbnail_small.jpg','https://www.manhal.com/platform/games/###/images/thumb.jpg') as thumbnail,media.title_ar,media.title_en,media.path, categories.name_ar as category_ar, categories.name_en as category_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type" . $typesmedia . $this->keyword . $this->category.$this->grade . " " . " ORDER BY `media`.`price` ASC,`media`.`id` DESC " ;
    ...
    private function getstory($keyword,$category,$FromTo,$series, $search = '',$grade,$userid=0)
    {
        ...
        if ($userid != 0 ) {
            $this->userfilter = " OR (story.`userid` = " . $userid."  AND `story`.`status`=1)";
        }
        ...
        //$type_filter = " AND `story`.`type` in(2,3,6,7) AND `story`.`status`=1";
        $type_filter = " AND `story`.`type` in(2,3)  AND `story`.`status`=1";


        $sql = "SELECT IF(story.storyid>0,'17','17') as type, (story.storyid)as id,story.seriesid,IF(story.thumb!='','https://www.manhal.com/platform/stories/$$$/story/###/images/pic.jpg','')as thumbnail,story.title as title_ar,story.title as title_en,story.catid as category ,`stories_cat`.`name_ar` as `category_ar` ,`stories_cat`.`name_en` as `category_en`   FROM `story` left OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid` left OUTER JOIN `series` on `story`.`seriesid`=`series`.`seriesid` WHERE `story`.`package`=0  AND `story`.`status`=1  AND( `story`.`storyid`>0 " . $this->keyword . $this->category . $this->series_filter_stories . $this->type . $type_filter.$this->grade . " ) ".$this->userfilter." " ;
