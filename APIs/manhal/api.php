    ...
    public function ForgotPassword($email, $lang = 'En')
    {
        ...
        if (mysqli_num_rows($result) > 0) {
           ...
           // return 'Error Email';
            ...
            return ["response" =>["result"=>0,"Message"=>'Error Email']];
        }
       ...
       // return $data_array;
        ...
        if (mail($to, $subject, $message, $headers)) {
            return ["response" =>["result"=>1,"Message"=>$data_array]];

           // return 'Send Email';
        } else {
          //  return 'Cannot Send Email';

            return ["response" =>["result"=>0,"Message"=>'Cannot Send Email']];
    ...
    public function Membership($userid)
    {
        ...
        switch ($row['permession']) {
            case 10:
                ...
                break;
            case 9://new work around to activate users with subscribtion
                $arrays['is_expired'] = '1';
                $sqlcod = "SELECT count(*)as counts,payment_subscribe.*,payments.payment_date FROM `payment_subscribe` LEFT JOIN payments ON `payments`.`paymentid`=`payment_subscribe`.`paymentid` WHERE payment_subscribe.`paymentid`=439";
                $resultcod = $con->query($sqlcod);
                $rowcod = mysqli_fetch_assoc($resultcod);
                if ($rowcod['counts'] >0) {
                    $arrays['info'] = $resultcod;
...
/*
    ...
    private function FilterStories($filter, $FromTo, $No)
    {
        ...
        $type_filter = " AND `story`.`type` in(2,3)";
        $story = "story.is_playlist, (story.storyid)as id,story.seriesid,IF(story.thumb!='','https://www.manhal.com/platform/stories/$$$/story/###/images/pic.jpg','')as thumbnail,`story`.`rating_count` as rate,story.title as title_ar,story.title as title_en,story.price,story.rate,story.view_count as view,`story`.description_ar,`story`.description_en";
        switch ($filter) {
            case 'toprated':
                $sql = "Select  " . $story . " From   `story`  INNER JOIN   stories_cat categories1 On story.catid = categories1.catid WHERE story.status=1 and  `story`.`package`=0 " . $type_filter . " Group By   story.storyid Order By   story.rate Desc   " . $FromTo;
                break;
            case 'mostviewed':
                $sql = "Select  " . $story . " From  story story Inner Join   stories_cat categories1 On story.catid = categories1.catid WHERE story.status=1 and  `story`.`package`=0  " . $type_filter . " Group By   story.view_count Order By   view_count Desc  " . $FromTo;
                break;
            case 'recent':
                $sql = "SELECT " . $story . "  FROM `story` INNER JOIN `stories_cat` on `story`.`catid`=`stories_cat`.`catid` WHERE `story`.`status`=1 and  `story`.`package`=0 " . $type_filter . " GROUP BY `story`.`storyid` order by storyid DESC  " . $FromTo;
                break;
            case "bestseller":
                $sql = "SELECT " . $story . "  FROM `story` INNER JOIN `stories_cat` on `story`.`catid`=`stories_cat`.`catid` WHERE `story`.`status`=1 and  `story`.`package`=0 " . $type_filter . " GROUP BY `story`.`storyid` order by storyid DESC  " . $FromTo;
                break;
            case "topfavorite":
                $sql = "Select " . $story . " From   `story`  INNER JOIN   stories_cat categories1 On story.catid = categories1.catid  WHERE story.status=1 and  `story`.`package`=0 " . $type_filter . " Group By   story.storyid Order By   story.favorite_count Desc   " . $FromTo;
                break;
            case 'ar':
                $sql = "SELECT " . $story . "  FROM `story` INNER JOIN `stories_cat` on `story`.`catid`=`stories_cat`.`catid` WHERE `story`.`status`=1 and  `story`.`package`=0  And `story`.`language`='Ar' " . $type_filter . " GROUP BY `story`.`storyid` order by storyid DESC  " . $FromTo;
                break;
            case 'en':
                $sql = "SELECT " . $story . "  FROM `story` INNER JOIN `stories_cat` on `story`.`catid`=`stories_cat`.`catid` WHERE `story`.`status`=1 and  `story`.`package`=0  And `story`.`language`='En' " . $type_filter . " GROUP BY `story`.`storyid` order by storyid DESC  " . $FromTo;
                break;
            case 'categories':
                $sql = "SELECT " . $story . "  FROM `story` INNER JOIN `stories_cat` on `story`.`catid`=`stories_cat`.`catid` WHERE `story`.`status`=1 and  `story`.`package`=0   AND `story`.`catid` ='" . $No . "' " . $type_filter . " GROUP BY `story`.`storyid` order by storyid DESC  " . $FromTo;
                break;
            default:
                return 'error';
                break;
    ...
    private function FilterMedia($filter, $FromTo, $typesmedia, $No)
    {
        global $con;
        $this->conn = $con;
        $Media = "media.id,media.is_playlist,IF(media.path='','https://www.manhal.com/platform/media/###/thumbnail_small.jpg','https://www.manhal.com/platform/games/###/images/thumb.jpg') as thumbnail,media.filename,media.price,media.rate,media.title_ar,media.title_en,media.views,media.path,`media`.description_ar,`media`.description_en";
        switch ($filter) {
            case 'toprated':
    ...
    private function RelatedStoriesFilter($id,$FromTo)
    {
        ...
        $this->type = " AND `story`.`type`in(2,3)";
        switch ($filtercount) {
            case 0:
    ...
    {
        $sql = "Select  media.*,IF(media.is_story='0',media.path,CONCAT('https://www.manhal.com/en/story/',media.filename,'/')) as path, categories.name_ar, categories.name_en From media Inner Join categories On media.category = categories.catid   where   media.type=" . $type . " and  media.status=1 and media.id=" . $id;
        $result = $this->conn->query($sql);
        return $result;
    }
    ...
    private function getbook($keyword, $category, $FromTo, $lang, $booktype, $search = '')
    {
        ...
        $this->category = '';
        $this->lang = '';
        $this->type = ' AND `books`.`booktype`>1';
        if ($keyword != "") {
            if ($search == 1) {
...
//`story`.`status`=1 AND
    ...
    private function getmedia($typesmedia, $keyword, $category, $FromTo, $search = '')
    {
        ...
        $sql = "Select  media.id,media.is_playlist,IF(media.path='','https://www.manhal.com/platform/media/###/thumbnail_small.jpg',IF(`media`.`is_story`=1,'https://www.manhal.com/platform/stories/$$##/images/pic.jpg','https://www.manhal.com/platform/games/###/images/thumb.jpg')) as thumbnail,media.price,media.rate,media.title_ar,media.title_en,media.views,media.path,`media`.description_ar,`media`.description_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type=" . $typesmedia . $this->keyword . $this->category . " " . " ORDER BY `media`.`price` ASC,`media`.`id` DESC " . $this->FromTo;
        $result = $this->conn->query($sql);
        return $result;
    }
    ...
    private function SocialLogin($social, $name, $fname, $lname, $gender, $email, $avatar)
    {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($this->conn->query($sql)) {
                return $this->SocialLogin($social, $name, $fname, $lname, $gender, $email);
            } else {
