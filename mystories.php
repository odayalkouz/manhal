                        ...
                        <div class="right-col-3 floating-left">
                            <div class="books-main-container-page clear-both">
                                ...
                                if($_SESSION['user']['permession'] < 6 && $_SESSION['user']['permession']>0  || $_SESSION['user']['userid']<5){
                                    $whereUser="`story`.`userid` > 0 ";
                                }else{
                                    $whereUser="`story`.`userid`=" . $_SESSION['user']['userid'];
                                }
                                $keyword_filter = "";
                                if (isset($_GET['keywords']) && $_GET['keywords'] != "") {
                                    $keyword=mysqli_real_escape_string($con,str_replace(" ","%",$_GET['keywords']));
                                    $keyword_filter = " AND ( `title` LIKE '%".$keyword."%' OR `description_ar` LIKE '%" . $keyword. "%' OR `description_en` LIKE '%" . $keyword. "%' OR `author_en` LIKE '%" .$keyword. "%' OR `author_ar` LIKE '%" .$keyword. "%' )";
                                }

                                $sql = "SELECT `story`.*,`stories_cat`.* FROM `story` left OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid` WHERE ".$whereUser.$keyword_filter;
                                $result = $con->query($sql);
                                $num_rows = mysqli_num_rows($result);

                                $url = "mystories?";
                                $pagination = getPagination($url, $num_rows, 11);
                                $sql = "SELECT `story`.*,`stories_cat`.* FROM `story` left OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid` WHERE ".$whereUser.$keyword_filter. " " . $pagination[0];
                                $result = $con->query($sql);
                                ...
                                <div class="ui-panels-worksheet">
                                    <div class="item-container jq_item_container floating-left tsc_ribbon_wrap reveal-bottom">
                                        ...
                                        if($_SESSION['user']["permession"]>0 && $_SESSION['user']["permession"]<=11){
                                            ?>
                                            <a class="inner-item-container addquiz"   href="<?= SITE_URL . $lang_code . "/story-editor?id=new"; ?>">
                                                <?= $Lang->AddStory; ?>
                                            </a>
                                            <?php
                                        }else{
