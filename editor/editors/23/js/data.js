...
function drawlistArray() {
    ...
    //     }
    // }
    for (var x=0;x<itemList2.length;x++){
        itemSelct.push(itemList2[x]);
    }
...
function drawItem(){
    ...
    for(var i=0;i<itemSelct.length;i++){
        ...
        html+='<div class="text-container" data="'+itemSelct[i].type+'">' +
              ...
              '<div class="trueFalse">' +
              '<a class="true-btn '+selected_true+'" data="1" onclick="choseCorrect(this)"></a>' +
              '<a class="false-btn '+selected_false+'" data="0" onclick="choseCorrect(this)"></a>' +
              '</div>' +
              '</div>'
    }
