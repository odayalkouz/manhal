...
<script>
        ...
        itemList: [
            {
                ...
                dropSiz: [{w: '7.7%', h: '42%', t: '33%', r: '78.9%', type: '1.1'}],
                id: 'id123'
            }
        ]
        ,
    ...
    $(window).load(function () {
        ...
        $(document).on("click",".jq_savei",function () {
            ...
            $(this).closest(".item-container").find(".jq_itemc").html(" "+$(this).closest(".item-container").find(".jq_itemc").html()+" ");
        ...
        $(document).on("mouseup",".jq_itemc",function (e) {
            if($(this).attr("contenteditable")=='false' || $(this).attr("contenteditable")==false){
                ...
                if(width>0){
                    var left=(((e.pageX-$(this).closest('.item-container').offset().left)/$(this).closest('.item-container').width()*100)-0.7).toFixed(2);
                }else{
                    var left=(((window.mousedown-$(this).closest('.item-container').offset().left)/$(this).closest('.item-container').width()*100)-0.7).toFixed(2);
                }
                ...
                $(this).closest('.item-container').append('<div class="drop-item ui-droppable" data="'+index+'" style="width: '+rwidth+'%;left: '+left+'%"><span></span></div>')
                var iclass='jq_itemc_'+$(this).closest('.item-container').attr('temp-id');
                $(".drag-container").append('<div class="drag-item '+iclass+'" iw="'+rwidth+'" il="'+left+'" style="margin: 10.94% 1.7% 13.9% 2.3%; width: 46.8%; height: 10%; float: none;"><div class="drag-item-inner"><span data="0" class="ui-draggable ui-draggable-handle" style="position: relative;">'+str+'</span><div class="trueanswer" title="true answer"></div></div></div>');
            }
        });

        $(document).on("click",".jq_addanswer",function () {
            $(".drag-container").append('<div class="drag-item" data="" style="margin: 10.94% 1.7% 13.9% 2.3%; width: 46.8%; height: 10%; float: none;"><div class="drag-item-inner"><a class="jq_deleteanswer"><a/><span data="0" class="ui-draggable ui-draggable-handle jq_editable" contenteditable="true">اجابة</span></div></div>');
        });
