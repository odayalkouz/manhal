...
//scorm
    ...
    $('body').manhalLoader({
        ...
        addFiles: [
            ...
            {type:'image',url:'images/correct.svg'},{type:'image',url:'images/wrong.svg'}
        ],
...
function choseCorrect(obj) {
    $(obj).closest('.trueFalse').find('a').removeClass('selected');
    // $(obj).parent().find("a").css("pointer-events","none");
    if($(obj).attr("data")==$(obj).parent().parent().attr("data")){
        $(obj).addClass("selected");
        correct++;
      //  soundEffect("sounds/click.mp3","","");
        if(questionNum==(itemSelct.length-1)){
