...
function drawItem() {
    ...
    for(var i=0;i<shuffleList2.length;i++){
        if(shuffleList2[i].card1.imgShow=="true" || shuffleList2[i].card1.imgShow==true){
            html1+='<div onclick="choseCorrect(this)" class="card cardClick" bk="'+shuffleList4[i].borderColor+'" bk2="'+shuffleList4[i].bkColor+'" data="'+shuffleList2[i].type+'"><div class="image-container" cat="'+shuffleList2[i].card1.cat+'"><img src="'+shuffleList2[i].card1.imag+'"></div>' +
                '<div class="img-title"><span>'+shuffleList2[i].card1.title+'</span></div></div>';
        }else {
            html1+='<div onclick="choseCorrect(this)" class="card cardClick" bk="'+shuffleList4[i].borderColor+'" bk2="'+shuffleList4[i].bkColor+'" data="'+shuffleList2[i].type+'"><div class="image-container" cat="'+shuffleList2[i].card1.cat+'"><img src="'+shuffleList2[i].card1.imag+'"></div>' +
                '<div class="img-title"><span>'+shuffleList2[i].card1.title+'</span></div></div>';
        }
        // if(shuffleList2[i].card2.imgShow=="true" || shuffleList2[i].card2.imgShow==true){
        //     html2+='<div onclick="choseCorrect(this)" class="card cardClick" bk="'+shuffleList4[i].borderColor+'" bk2="'+shuffleList4[i].bkColor+'" data="'+shuffleList2[i].type+'"><div class="image-container" cat="'+shuffleList2[i].card2.cat+'"><img src="'+shuffleList2[i].card2.imag+'"></div>' +
        //         '<div class="img-title"><span>'+shuffleList2[i].card2.title+'</span></div><div style="width: 55%;right: auto" class="inner-card" bk="'+shuffleList4[i].bkColor+'"><span class="textSpan">'+shuffleList2[i].card2.text+'</span></div></div>';
        // }else {
        //     html2+='<div onclick="choseCorrect(this)" class="card cardClick" bk="'+shuffleList4[i].borderColor+'" bk2="'+shuffleList4[i].bkColor+'" data="'+shuffleList2[i].type+'"><div class="image-container" cat="'+shuffleList2[i].card2.cat+'"><img src="'+shuffleList2[i].card2.imag+'"></div>' +
        //         '<div class="img-title"><span>'+shuffleList2[i].card2.title+'</span></div><div class="inner-card" bk="'+shuffleList4[i].bkColor+'"><span class="textSpan">'+shuffleList2[i].card2.text+'</span></div></div>';
        // }
    ...
    $(".card").each(function () {
        if($(this).find(".image-container").attr("cat")=="1"){
            $(this).find(".image-container").css("height","100%");
        }else {
            // $(this).find(".img-title").fadeOut();
        }
    });
    $(".col1 .card .img-title").each(function () {
        if($(this).find("span").text()==""){
            $(this).parent().find(".image-container").css({"bottom":0})
        }
    })
