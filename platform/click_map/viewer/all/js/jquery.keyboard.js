...
;(function (factory) {
	...
	base.bindKeys = function () {
		...
		base
			...
			.bindButton(o.keyBinding.split(' ').join(base.namespace + ' ') + base.namespace + ' ' +
				...
				if($key.attr('data-action')=='undefined'||$key.attr('data-action')==undefined){
					$key.attr('data-action','X');
					action='x'
				}else{
					console.log("=====",$key.attr('data-action'))
					action = $key.attr('data-action').split('$').join(' ');
				}
				...
				if($key.attr('data-value')=='undefined'||$key.attr('data-value')==undefined){

					last.key = 'x'
				}else{
					last.key = $key.attr('data-value').split('$').join(' ');
				}
				...
				if($key.attr('data-name')==undefined||$key.attr('data-name')=='undefined'){
					last.key ='x'
				}else{
					last.key = $key.attr('data-name').split('$').join(' ');
				}
	...
	base.addKey = function (keyName, action, regKey) {
		...
		data.html = '<span class="' + kbcss.keyText + '">' +
			...
			'</span>';
		data.html =(data.html).split('$').join(' ');
		data.$key = base.keyBtn
			.clone()
			.attr({
				'data-value': regKey ? (keys.name).split('$').join(' ') : (keys.action).split('$').join(' '), // value
				'data-name': (keys.action).split('$').join(' '),
				'data-pos': base.temp[1] + ',' + base.temp[2],
				'data-action': (keys.action).split('$').join(' '),
				'data-html': data.html
			})
