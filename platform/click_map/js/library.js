...
function createThumbs() {
    ...
    strContainer = '<div class="libraryContainer"><div class="libraryClass2">' +
        '<div class="headerFileInput2"><label>'+langRes.Library+'</label>' +
        ...
        '<div class="btn-save-container floating-right"><a class="ok-btn" onclick="removeLibraryBox()"><label>'+langRes.Save+'</label></a></div>' +
        '</div>' +
        '</div>' +
...
function UploadobjectLibrary(object) {
    ...
    ajaxPHP(pathSrc, "", "CopyFile", name, "Library",config.rootPath + "/all/images/" + name)
}
