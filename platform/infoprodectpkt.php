...
//
        ...
        $sql = "SELECT `payments_books`.*,`payments_books`.`type` as item_type,  `books`.`booktype` , `books`.`name`, `books`.`isbn` as book_isbn, `story`.`title` ,`story`.`isbn` as story_isbn, `products`.`name_ar`, `products`.`name_en` , `products`.`ISBN` as toy_isbn            
                                FROM `payments_books` LEFT OUTER JOIN `books` ON `payments_books`.`bookid`=`books`.`bookid` AND `payments_books`.`itemtype`='book'
                                 ...
                                 where payments_books.paymentid=".$_GET['id']." and ( payments_books.`type`=1 or payments_books.`type`=3 or payments_books.`type`=5 or payments_books.`type`=7 )";
...
//                                LEFT OUTER JOIN `categories` ON `books`.`category`=`categories`.`catid` LEFT OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid`  where payments_books.paymentid=".$_GET['id']." and ( payments_books.`type`=1 or payments_books.`type`=3 or payments_books.`type`=5 or payments_books.`type`=7 )" . $pagination[0];
        ...
        $sql = "SELECT `payments_books`.*,`payments_books`.`type` as item_type,  `books`.`booktype` , `books`.`name`, `books`.`isbn` as book_isbn, `story`.`title` ,`story`.`isbn` as story_isbn, `products`.`name_ar`, `products`.`name_en` , `products`.`ISBN` as toy_isbn                                FROM `payments_books` LEFT OUTER JOIN `books` ON `payments_books`.`bookid`=`books`.`bookid` AND `payments_books`.`itemtype`='book'
                                  LEFT OUTER JOIN `story` ON `payments_books`.`bookid`=`story`.`storyid` AND `payments_books`.`itemtype`='story'
                                  LEFT OUTER JOIN `products` ON `payments_books`.`bookid`=`products`.`productid` AND `payments_books`.`itemtype`='toy'
                                 where payments_books.paymentid=".$_GET['id']." and ( payments_books.`type`=1 or payments_books.`type`=3 or payments_books.`type`=5 or payments_books.`type`=7 )". $pagination[0];
