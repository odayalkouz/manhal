...
//new script for scorm - By Hussam
    correctsum2 = Math.round(correctsum - incorrectsum);
...
var k=String($("#yourscorenum").html()).split('/')[0];

    if (LMSStatus) {
        API.SetValue("cmi.score.max", quiz_data.question.fullscore);//to set max score in the game
        API.SetValue("cmi.score.raw",  k);//return text true or false | to set student mark
        API.SetValue("cmi.completion_status", "completed");//when complete game
        API.SetValue("cmi.success_status", finalR);//when complete game set value to one of ("passed","failed","unknown")
        API.SetValue("cmi.session_time", $("#timerCount").html());//to set Amount of seconds that the learner has spent
...
function checkonline(){
    ...
    if(parent.$("#popup-container").attr("user-type")=="student"){
        var quiz_id=parent.$('#quiz-viewer').attr('quizid');
        var id_assign = parent.$(parent.$(".vertical-slider-viewer ul li.selected")).attr("media_id");
        var urlschool='https://imanhal.manhal.com//quizmedia/is-applying-exam?quiz_id='+quiz_id+'&id_assign='+id_assign
        console.log(" 1= "+urlschool)
        $.ajax({
            url: urlschool,
            type: "GET",
            cache: false,
            ...
            success: function (html) {
                if(html['data']['count']==0){
                  var studentname=  parent.$("#popup-container").attr("name");
                    $("#studnetname").html(studentname);
                    $(".student-name-container").show();
                    createquiznow();
                }else{
                    $(".exam-is-finshed").show();
...
$(document).ready(function () {
    $(document).on("click", ".jq_bgsound", function () {
        bgsound_bath = $(this).attr("bgsound");
        var sound = new Howl({
            src: [bgsound_bath]
        });
        sound.play();
...
function initialize() {
     ...
     quiz_data = {'properties': {}, 'question': {}};
     random_question;
     correctsum = 0;
     incorrectsum = 0;
     timerPause = false;

     Object_quizinfo = xmlDoc.getElementsByTagName("properties")[0].getElementsByTagName("quizinfo")[0];
...
console.log("----",myVar233)
  ...
  random_question = new Array(quiz_data.question.quiz.length);
   for (var i = 0; i <quiz_data.question.quiz.length; i++) {
       console.log(i)
       random_question[i]=i;
   }
    ...
    for (var i = 0; i < random_question.length; i++) {
        ...
        switch (typeQ) {
            ...
            case '7':
                //Click Map
                $("#ClickMap" + i).off();
                $("#ClickMap" + i).on("click", function (event) {
    ...
    $(".image:not(.asound) .real-content img").addClass("image-link-popup");
...
function callfunctionCorrection() {
    ...
    for (var i = 0; i < random_question.length; i++) {
        ...
        if (flag == true) {
            ...
            console.log(correctsum,'-----',point.toFixed(1))
            correctsum += parseFloat(point.toFixed(1));
            console.log(correctsum)
...
function dragable(i, drop) {
    $("#" + i).draggable({
        ...
        drag: function (event, ui) {
            $(".questions-main-container .question-answer-container .answer .word-bank .box-drag-in").css('color', 'transparent');
        },
        start: function () {
        ...
        stop: function () {
            $(this).css('z-index', '2');
            $(".questions-main-container .question-answer-container .answer .word-bank .box-drag-in").css('color', '#575757');
        }
    });
    $("#" + drop).droppable({
...
function resizeviews() {
    setTimeout(function () {
        $(".clicksss").each(function (index) {
            ...
            var ll = (100 * parseFloat($(this).css("left")) / parseFloat($(this).parent().css("width"))) + "%";
            var tt = (100 * parseFloat($(this).css("top")) / parseFloat($(this).parent().css("height"))) + "%";
            $(this).css("left", ll);
            $(this).css("top", tt);
        });

        $(".resizables").each(function (index) {
            $(this).outerWidth($(this).outerWidth() / ($(this).parent().outerWidth()) * 100 + "%");
            $(this).outerHeight($(this).outerHeight() / ($(this).parent().outerHeight()) * 100 + "%");
            if ($(this).offset().top - $(this).parent().offset().top + $(this).outerHeight() > $(this).parent().outerHeight()) {
                $(this).outerHeight((($(this).parent().outerHeight() - ($(this).offset().top - $(this).parent().offset().top)) / $(this).parent().outerHeight()) * 100 + "%");
...
function typeqyiz(i) {
    ...
    switch (String(typeQ)) {
        ...
        case '7':
            ...
            data += ' <div class="click-map floating-left swiper-no-swiping"> <div class="pointer-click"></div> <div class="box-map-main"> <div  class="box-map-inner" id="ClickMap' + i + '"> <div class="resizables" style="aaaa;float:left;overflow: hidden;position: relative;width:' + answer[0].width + 'px; height:' + answer[0].height + 'px; left:' + answer[0].left + ';"><div id="ErrorClickMap' + i + '"></div>';

            for (var j = 0; j < answer[0].answer.length; j++) {
                data += "<div class='clicksss ccc' onclick='shopoint(this.id)'  id='map_" + i + '_' + j + "' map='false' style='z-index:9999;width:" + answer[0].answer[j].W + "px; height:" + answer[0].answer[j].H + "px;position:absolute;left:" + answer[0].answer[j].L + "px; top:" + answer[0].answer[j].T + "px'><div class='bullet'> <div class='line zero'></div> <div class='line one'></div> <div class='line two'></div> <div class='line three'></div> <div class='line four'></div> <div class='line five'></div> <div class='line six'></div> <div class='line seven'></div></div></div>";
            }
        ...
        case '9':
            //Sequence
            data += '<div id="SQuastion' + i + '" class="swiper-slide ' + i + '"> <div class="question-answer-container"> <div class="question-view"><div class="display-none1" id="CorQuastion' + i + '" ><div class="marknum">1/10</div></div> <div class="num floating-left">' + (i + 1) + '</div> <div class="text floating-left">';
            data += Quastion + ' </div> </div><div class="answer swiper-no-swiping"><div  class="sequence floating-left"><ul class="floating-left"> ';
            for (var x = 0; x < answer.length; x++) {
                data += '<li class="line-row"><label class="box-number floating-left">' + parseInt(x+1) + '</label></li>';
            }
            data += '</ul>';
            data +='<ul class="floating-left" id="sortable' + i + '" > ';
            var answerRandom = randomarray(answer.length);
            for (var j = 0; j < answer.length; j++) {
                data += '<li class="line-row swiper-no-swiping ui-state-default" id="Quastion' + i + '_' + j + '" att="' + answerRandom[j] + '"><span class="floating-left">' + answer[answerRandom[j]].answer + '</span> </li>';
            }
            data += '</ul></div> </div></div>';
            break;
    }
