...
$bredcrumb = '<li class="floating-left"><a href="games.php" class="floating-left active">'.$Lang->Games.'</a></li>';
    ...
    <div class="books-container">
        ...
        <div class="display-table">
            ...
            if(isset($_GET['category']) && $_GET['category']!=0){
                ...
                $cat_filter=" AND `editors`.`category` = ".$_GET['category'];

            }



            $sql = "SELECT `editors`.*,`users`.*,`categories`.* FROM `editors` left OUTER JOIN `categories` ON `editors`.`category`=`categories`.`catid` left OUTER JOIN `users` on `editors`.`userid`=`users`.`userid` WHERE `editors`.`gameid` >0 " . $weruser .$keyword_filter.$cat_filter;
            ...
            $sql = "SELECT `editors`.*,`users`.*,`categories`.* FROM `editors` left OUTER JOIN `categories` ON `editors`.`category`=`categories`.`catid` left OUTER JOIN `users` on `editors`.`userid`=`users`.`userid` WHERE `editors`.`gameid` >0 " . $weruser .$keyword_filter.$cat_filter.$pagination[0];
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $data .= "<div id='gameid_".$row['gameid']."' class='display-table-row #class#'>";

                    $data .= "<div class='display-table-cell number' >".$page_number."</div>";

                    $data .= "<div class='display-table-cell book-title'><a href='".$row['editor']."/index.php?id=".$row['gameid']."'>".$row['title_en']."</a></div>";
                    ...
                    $data .= "<div class='display-table-cell book-thumb'><a class='preview'><img src='games/".$row['gameid']."/images/thumb.jpg' alt='".$row['title_en']."' /></a></div>";
                    ...
                    if (canEditGame($row)) {
                        ...
                        $data .= " <a title='" . $Lang->Info . "' href='editgame.php?gameid=".$row["gameid"]."'>";

                        $data .= " <i class='flaticon-info27'></i> </a>";

                        $data .= " <a title='" . $Lang->Delete . "'  class='deletegame' gameid='".$row["gameid"]."' >";

                        $data .= " <i class='flaticon-delete96'></i> </a>";

                        $data .= '<a title="' . $Lang->Download . '" class="" target="_blank" href="ajax/download.php?gameid='.$row["gameid"].'">';

                        $data .= " <i class='flaticon-download195'></i></a>";

                        $data .= '<a title="' . $Lang->Publish . '" class="jq_publishgame" data-href="ajax/games.php?process=publishgame&gameid='.$row["gameid"].'">';

                        $data .= " <i class='flaticon-arrow73'></i></a>";

                            $data .= '<a title="' . $Lang->View . '" target="_blank" href="' . $row['editor'] . '/viewer/' . strtolower($row['language']) . '/index.php?id=' . $row["gameid"] . '" >';
                        

                        $data .= " <i class='flaticon-eye106'></i></a>";
                        if($row["editor"]!='coloring') {
                            $data .= '<a title="' . $Lang->Edit . '" href="' . $row['editor'] . '/index.php?id=' . $row['gameid'] . '">';
                    ...
                    $data .= '<a title="' . $Lang->Copy . '" class="jq_dublicategame" href="editgame.php?id=new&copy='.$row["gameid"].'">';
        ...
        <a href="editgame.php?id=new" class="btn-default floating-right"><?=$Lang->AddGame;?></a>
