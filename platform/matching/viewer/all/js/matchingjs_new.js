...
function getPosition(e,p) {
    ...
    if (!is_touch_device()) {
        ...
        var x = event.pageX - p.offset().left;
        var y = event.pageY - p.offset().top;
    }
    else {
        var x = event.touches[0].pageX - p.offset().left;
        var y = event.touches[0].pageY - p.offset().top;
...
mouse_down = function (e) {
    ...
    if (e.touches == undefined) {
        ...
        x = getPosition(e,$(this.canvas)).x;
        y = getPosition(e,$(this.canvas)).y;
    }
    this.x = (x);
    this.y = (y );
