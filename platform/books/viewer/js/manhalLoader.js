...
(function ($) {
    ...
    var getImages = function (element) {
        $(element).find('*:not(script)').each(function () {
            var url = "";
            var urlSound = "";
            if ($(this).css('background-image').indexOf('none') == -1 && $(this).css('background-image').indexOf('linear-gradient') == -1 && $(this).css('background-image').indexOf('-webkit-linear-gradient') == -1 && $(this).css('background-image').indexOf('-o-linear-gradient') == -1 && $(this).css('background-image').indexOf('-mos-linear-gradient') == -1) {
    ...
    var loadComplete = function ()
    {
        ...
        }, 500, 'linear', function ()
        {
            ...
            $("#manhalpreOverlay").fadeOut("slow");

            $(".viewer-container .first-section-viewer .title h1").addClass("animated zoomIn").fadeIn();
                    setTimeout(function () {
                      $(".viewer-container .first-section-viewer .information .box-a").fadeIn();
                      setTimeout(function () {
                        $(".viewer-container .first-section-viewer .information .box-a span").addClass("animated puffIn").fadeIn();
                        setTimeout(function () {
                            $(".viewer-container .first-section-viewer .information .box-b ").fadeIn();
                            setTimeout(function () {
                                $(".viewer-container .first-section-viewer .information .box-b span").addClass("animated puffIn").fadeIn();
                                setTimeout(function () {
                                    $(".viewer-container .first-section-viewer .information .box-c").addClass("animated puffIn").fadeIn();
                                    setTimeout(function () {
                                        $(".viewer-container .first-section-viewer .information .play").addClass("animated flip").fadeIn();
                                        setTimeout(function () {
                                            $(".book-play").addClass("animated fadeInRight").fadeIn();
                                        },350);
                                    }, 450);
                                }, 450);
                            }, 450);
    ...
    var createContainer = function (tar) {
        jOverlay = $('<div><div class="logo-loader"></div></div>')
