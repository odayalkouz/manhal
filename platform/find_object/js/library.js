...
LibraryFolderName = ['shape', 'alphabet','animal', 'education', 'faces', 'people', 'social', 'symbole', 'tools', 'Stamps']
LibraryThumbName = [
...
function createThumbs() {
    ...
    strContainer = '<div class="libraryContainer"><div class="libraryClass2">' +
        '<div class="headerFileInput2"><label>Library</label></div>' +
        '<div class="libraryWrpper">' +
        '<img type="button" style="position: absolute; right: 10px; z-index: 999999999999; /* width: 38px; */ top: 7px; cursor: pointer; " src="images/close.png" onclick="removeLibraryBox()">' +
        '<div class="category">' +

        '' +
        '' +
        '</div>' +
        '<div class="contentCatg infinite-scroll">' +
        '' +
        '' +
        '</div>' +
        '</div>' +
...
function UploadobjectLibrary(object) {
    ...
    pathSrc = "../games/" + pathSrc
    array = (pathSrc.split('/'))
    name = array[array.length - 1]
    ajaxPHP(pathSrc, "", "CopyFile", name, "Library",config.rootPath + "/images/" + name)
}