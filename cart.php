...
<link rel="stylesheet" type="text/css" href="<?=SITE_URL;?>themes/main-Light-green-<?=$_SESSION['lang'];?>/css/cart.css<?=$cash;?>">
        ...
        $(".BtnNext").click(function () {
            ...
            if (!$(this).hasClass("btn-popup")) {
                if (i == 0 && window.total == 0) {
                    ...
                    var msg = "";
                    if (($("#cart_email").val().trim()).length < 4 || !isEmail($("#cart_email").val().trim())) {
                        msg = window.Lang.InvalidEmailAddress + "<br>";
                        $("#cart_email").addClass("error-feild").focus();
                    }
...
<div class="inner-pages-main-container-a cart-main-container">
    ...
    <div class="center-piece">
      <?php
      if(isset($_SESSION["user_country"]) && $_SESSION["user_country"]=='jordan'){
        ?>
                        ...
                        <div class="row">
                          ...
                          if (count($items["book"]) > 0) {
                              ...
                              while ($row = mysqli_fetch_assoc($result)) {
                                  ...
                                  $viewLink="";
                                    echo '<div class="col-xl-12 col-md-12 d-sm-flex justify-content-between my-3 pb-3 border-bottom  row-container cart_row" id="order_book_'.$row["bookid"].'" data-weight="'.$row["weight"].'" data-type="book" data-id="'.$row["bookid"].'">  <div class="media d-block d-sm-flex text-center text-sm-left" >
                                      <a class="cart-item-thumb mx-auto mr-sm-4" ><img src="'.SITE_URL.'platform/books/'.$row['bookid'].'/cover.jpg" alt="Product"></a>
                                      <div class="media-body pt-2">
                                          <h3 class="product-card-title font-weight-semibold border-0 pb-0"><a>'.$row['name'].'</a></h3>
                                          <div class="font-size-sm"><span class="mr-2">'.$Lang->Category.':</span><span class="text-muted mr-2">'. $row["name_" . $cat_code].'</span></div>
                                          <div class="font-size-lg text-muted1 pt-2 ">$'.$item_price.'</div>
                                      </div>
                                  </div>  <div class="pt-2 pt-sm-0 pl-sm-3 mx-auto mx-sm-0 text-center text-sm-left" style="max-width: 10rem;">
                                        <div class="form-group mb-2">
                                            <label for="quantity1" class="quantity2">'.$Lang->Quantity.'</label>
                                            <input class="form-control form-control-sm QTY book_qty" type="number" item_type="book" item_price="'.$item_price.'" id="quantity1" min="1" value="'.$items["book"][$row["bookid"]]["quantity"].'">
                                        </div>
                                        <a class="btn btn-sm mb-2 text-right Remove-Item float-right jq_delete_carts_item" type="button">'.$Lang->removeItem.'</a>
                                    </div>
                                </div>';
                            ?>
                                  <?php
                                                      ...
                                                      if (count($items["story"]) > 0) {
                                                          ...
                                                          while ($row = mysqli_fetch_assoc($result)) {
                                                              ...
                                                              $viewLink="";
                                                              echo '<div class="col-xl-12 col-md-12 d-sm-flex justify-content-between my-3 pb-3 border-bottom  row-container cart_row" id="order_story_' . $row["storyid"] . '" data-weight="'.$row["weight"].'" data-type="story" data-id="'.$row["storyid"].'">  <div class="media d-block d-sm-flex text-center text-sm-left" >
                                                                <a class="cart-item-thumb mx-auto mr-sm-4" ><img src="'.SITE_URL.'platform/stories/'.$row['seriesid'].'/story/'.$row['storyid'].'/images/pic.jpg" alt="Product"></a>
                                                                <div class="media-body pt-2">
                                                                    <h3 class="product-card-title font-weight-semibold border-0 pb-0"><a >'.$row['title'].'</a></h3>
                                                                    <div class="font-size-sm"><span class="mr-2">'.$Lang->Category.':</span><span class="text-muted mr-2">'. $row["name_" . $cat_code].'</span></div>
                                                                    <div class="font-size-lg text-muted1 pt-2 ">$'.$item_price.'</div>
                                                                </div>
                                                            </div>  <div class="pt-2 pt-sm-0 pl-sm-3 mx-auto mx-sm-0 text-center text-sm-left" style="max-width: 10rem;">
                                                                  <div class="form-group mb-2">
                                                                      <label for="quantity1" class="quantity2">'.$Lang->Quantity.'</label>
                                                                      <input class="form-control form-control-sm QTY book_qty" item_price="'.$item_price.'" item_type="story" type="number" id="quantity1" min="1" value="'.$items["story"][$row["storyid"]]["quantity"].'">
                                                                  </div>
                                                                  <a class="btn btn-sm mb-2 text-right Remove-Item float-right jq_delete_carts_item" type="button">'.$Lang->removeItem.'</a>
                                                              </div>
                                                          </div>';
                                                                                ...
                                                                                if (count($items["toy"]) > 0) {
                                                                                    ...
                                                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                                                        ...
                                                                                        $viewLink="";
                                                                                        echo '<div class="col-xl-12 col-md-12 d-sm-flex justify-content-between my-3 pb-3 border-bottom  row-container cart_row" id="order_toy_' . $row["productid"] . '" data-weight="'.$row["weight"].'" data-type="toy" data-id="'.$row["productid"].'">  <div class="media d-block d-sm-flex text-center text-sm-left" >
                                                                                          <a class="cart-item-thumb mx-auto mr-sm-4" ><img src="'.SITE_URL.'platform/products/'.$row['productid'].'/thumbnail_small.jpg" alt="Product"></a>
                                                                                          <div class="media-body pt-2">
                                                                                              <h3 class="product-card-title font-weight-semibold border-0 pb-0"><a >'.$row["name_" . $cat_code].'</a></h3>
                                                                                              <div class="font-size-sm"><span class="mr-2">'.$Lang->Category.':</span><span class="text-muted mr-2">'. $row["name_" . $cat_code].'</span></div>
                                                                                              <div class="font-size-lg text-muted1 pt-2 ">$'.$item_price.'</div>
                                                                                          </div>
                                                                                      </div>  <div class="pt-2 pt-sm-0 pl-sm-3 mx-auto mx-sm-0 text-center text-sm-left" style="max-width: 10rem;">
                                                                                            <div class="form-group mb-2">
                                                                                                <label for="quantity1" class="quantity2">'.$Lang->Quantity.'</label>
                                                                                                <input class="form-control form-control-sm QTY book_qty" item_price="'.$item_price.'" item_type="toy" type="number" id="quantity1" min="1" value="'. $items["toy"][$row["productid"]]["quantity"].'">
                                                                                            </div>
                                                                                            <a class="btn btn-sm mb-2 text-right Remove-Item float-right jq_delete_carts_item" type="button">'.$Lang->removeItem.'</a>
                                                                                        </div>
                                                                                    </div>';
                           ...
                           ?>
                            ...
                            <div class="col-xl-12 col-md-12">
                                <div class="row">
                                    ...
                                    <div class="col-xl-6 col-md-6 text-right subTotal1 cart_total_price"><?= $Lang->TotalPrice; ?> : $<?=$total_price;?></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row"><hr style="border-top: 1px solid #eee;"></div>
                    <!--REVIEW ORDER END-->
                    </div>
