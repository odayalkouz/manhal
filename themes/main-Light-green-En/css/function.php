                             ...
                             <a href="'.$viewLink.'" tabindex="0"></a>
                               <div class="games-item">
                                <a href="'.$viewLink.'" tabindex="0">
                                  <div class="game-thumb books-class" style="background-image: url('.$bg.')"></div>
                                    <div class="game-title">
                                                <label>'.$book['name'].'</label>
                                            </div>
                                            <div class="title-sub-container clear-both floating-right">
                                                <div class="floating-right display-inline-block ">
                                                       <div class="right floating-right"><div class="display-inline-block"><span class="floating-right">7.05</span><span class="floating-right">$</span></div></div>
                                                    </div>
                                            </div>
                                            <div class="display-block top-container">
                                                <div class="hover-container floating-right">
                                                    <div class="buttons-container floating-left">
                                                        <a class="buy book_addtocart store floating-left" booktype="1" price="7.05" bookid="1190"></a>
                                                    </div>
                                                </div>
                                            </div>
                               </a>
...

{
    ...
    $sql = "SELECT `story`.*,`stories_cat`.*,`series`.*,`story`.`rating_count` as rate_count FROM `story` left OUTER JOIN `stories_cat` ON `story`.`catid`=`stories_cat`.`catid` left OUTER JOIN `series` on `story`.`seriesid`=`series`.`seriesid`  WHERE `story`.`status`=1 " . $filter . "  AND `story`.`storyid` NOT IN(" . $notIn . ") LIMIT 8";
...
function getShippingPrice($total_weight)
{
    ...
    switch ($total_weight) {
        ...
        case $total_weight <= 35:

            $price = $dhl_price_array["35"];
        ...
        case $total_weight <= 45:

            $price = $dhl_price_array["45"];
        ...
        case $total_weight <= 75:

            $price = $dhl_price_array["75"];

            break;

        case $total_weight <= 100:

            $price = $dhl_price_array["100"];
