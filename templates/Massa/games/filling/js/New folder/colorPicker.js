...
function startColorPicker() {
    ...
    canvasPicker.width = 300;
    canvasPicker.height =  300;
...
// http://www.javascripter.net/faq/rgbtohex.htm
    ...
    newX=newY=0
    function mouseMove(){
        radius=$(".canvasPicker").width()
        distance=9

        cx=(canvasPicker.width/2)+parseInt($(".canvasPicker").left())
        cy=(canvasPicker.height/2)+parseInt($(".canvasPicker").top())
        event.preventDefault()
        ...
        if(pointInCircle(x, y,cx ,cy ,radius )) {
            ...
            newX = cx + (radius * Math.cos(rad))+parseInt($(".canvasPicker").left())
            newY = cy + (radius * Math.sin(rad))+parseInt($(".canvasPicker").top())
