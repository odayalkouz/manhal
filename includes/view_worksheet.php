...
?>
<!--    fetchRelatedWorksheet($rows, $exist)-->
<?php
    ...
    <div class="wrapper">
        ...
        <ul class="tabs">
            ...
            <div class="center-piece">
                ...
                <li class="floating-left active"><?=$Lang->Worksheet;?></li>
                            ...
                            <div class="content__wrapper first-tab-content">
                                ...
                                <div class="top-container-a" style="padding: 13px 21px">
                                    ...
                                    <div class="cover-image-container floating-left scrollable" style="height: 581px;overflow-y: auto">
                                        ...
                                        if($row['price']==0 || Areyousubscribe(true)==1){ //covid-19 corona update :)

                                            $imageworksheet=SITE_URL . 'platform/media/' . $row['id'] . '/'.$row['filename'].'image_larg.jpg';

                                            $linkpdf ='href="'. SITE_URL .'platform/media/' . $row['id'] . '/' . $row['filename'] . '.pdf"';
                ...
                <div class="right-container floating-right">
                    ...
                    <div class="content-right">
                        ...
                        <div class="buttons-container">
                                ...
                                <?php
                        if($row['is_playlist']==0){
                             if($openworksheet==true){
                                 ...
                                 <a <?=$linkpdf;?> class="download-btn <?=$class?>" <?=$Cl2?> data-type="worksheet" bookid="<?=$row["id"];?>"><?=$Lang->Download;?></a>
                                 <?php
                             }else{
                                 ?>
                                 <a href="<?=SITE_URL.strtolower($session_lang);?>/subscribe" class="download-btn" ><?=$Lang->Download;?></a>
                                 <?php
                            }
                        }else{
                            if(Areyousubscribe(true)){
                                $playlist_link =SITE_URL.$lang_code.'/playlist/'.$row['id'].'/'.str_replace(" ","-",$row['title_'.$cat_code]);
                                ?>
                                <a class="jq_viewplaylist viewplaylist-icon" data-href="<?=$playlist_link;?>" title="<?=$Lang->PlayList;?>"></a>
                                <?php
                            }else{
                                $playlist_link =SITE_URL.$lang_code.'/subscribe';
                                ?>
                                <a class=" viewplaylist-icon" href="<?=$playlist_link;?>" title="<?=$Lang->PlayList;?>"></a>
                                <?php
                            }
                        }
                                ?>
                        ...
                        <div class="buttons-container-shar">
                            ...
                            if(!$detect->isMobile() || $detect->isTablet()) {
                                ...
                                <div class="right-button floating-right">
                                    <a class="without-icon floating-right show_comment " title="<?= $Lang->Comment; ?>"><label
                                                class="comment"></label></a>
                                    <a class="without-icon floating-right add_favorite <?= isWished($row["id"], "worksheet"); ?>"
                                       data-id="<?= $row["id"]; ?>" data-type="worksheet"
