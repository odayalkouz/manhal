                                    ...
                                    <div class="top-container-a">
                                        <div class="rating-container floating-left">
                                            ...
                                            <div class="stars floating-right">
                                                <form action="">
                                                    ...
                                                    <input <?=disableRate($row['id'],$row,'games');?> rate="5" bookid="<?=$row['id']?>" <?=calcRate($row['rate'],5);?> class="star star-5" id="star-5-<?=$id?>" prodect="games" type="radio" name="star" />
                                                    <label class="<?=msglogin(disableRate($row['id'],$row,'games'));?>  star star-5" for="star-5-<?=$id?>"></label>
                                                    <input <?=disableRate($row['id'],$row,'games');?> rate="4" bookid="<?=$row['id']?>" <?=calcRate($row['rate'],4);?> class="star star-4" id="star-4-<?=$id?>"  prodect="games" type="radio" name="star" />
                                                    <label class="<?=msglogin(disableRate($row['id'],$row,'games'));?> star star-4" for="star-4-<?=$id?>"></label>
                                                    <input <?=disableRate($row['id'],$row,'games');?>  rate="3" bookid="<?=$row['id']?>" <?=calcRate($row['rate'],3);?> class="star star-3" id="star-3-<?=$id?>"  prodect="games" type="radio" name="star" />
                                                    <label class="<?=msglogin(disableRate($row['id'],$row,'games'));?> star star-3" for="star-3-<?=$id?>"></label>
                                                    <input <?=disableRate($row['id'],$row,'games');?> rate="2" bookid="<?=$row['id']?>" <?=calcRate($row['rate'],2);?> class="star star-2" id="star-2-<?=$id?>"  prodect="games" type="radio" name="star" />
                                                    <label class="<?=msglogin(disableRate($row['id'],$row,'games'));?> star star-2" for="star-2-<?=$id?>"></label>
                                                    <input <?=disableRate($row['id'],$row,'games');?> rate="1" bookid="<?=$row['id']?>" <?=calcRate($row[  'rate'],1);?> class="star star-1" id="star-1-<?=$id?>"  prodect="games" type="radio" name="star" />
                                                    <label class="<?=msglogin(disableRate($row['id'],$row,'games'));?> star star-1" for="star-1-<?=$id?>"></label>
                                                    <!-- end khalid 5-9-2016 -->
                                                </form>
                                            </div>
                                        </div>
                                        <div class="cover-image-container floating-left">
                                            ...
                                            if($row['path']=='') {
                                                ...
                                                $imagegames=SITE_URL . 'platform/games/' . $idpath[1] . '/images/image_larg.jpg';
                                            }
                                            ...
                                            if($row['price']==0 || Areyousubscribe()==1 || true) {//covid-19 corona update :)
                                                $hrf="onclick='javascript:viewMedia(".$row['id'].");";
                                                if($row['is_newtab']==1){
                                                    $hrf.="'";
                                                }else{
                                                    $hrf.="javascript:launchFullscreen();'";
                                                }
                                            }
                                            ...
                                            <div class="games-view-demo">
                                                <div style="background-image:url(<?=$imagegames?>);" class="inner-games-view-demo" ></div>
                                            </div>
                                        ...
                                        <div class="buttons-container">
                                            ...
                                            <div class="right-button floating-right">
                                                ...
                                                <a class="without-icon floating-right add_favorite <?=isWished($row["id"],"games");?>"  data-id="<?=$row["id"];?>" data-type="games" title="<?=$Lang->Addtofavorite;?>"><label class="fav"></label></a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="content__wrapper third-tab-content">
                                    <div class="write-comment-container">
                                        ...
                                        if(isset($_SESSION["user"]) && !empty($_SESSION["user"]))
                                        {
                                            ...
                                            <a id="addcomment" data-type="games" data-id="<?=$row["id"];?>" class="floating-right comment"><?=$Lang->Comment;?></a>
                                            <?php
                                        }
                                        else
                                        {
                                        ...
                                        <div class="post-comment-container scrollable">
                                            ...
                                            $coment_array=getComments("games",$row["id"],1);
                                            echo $coment_array[0];
                                            ?>
                                        </div>
                                        <div class="pagination-container">
                                            <div class="paging">
                                                <div class="content" data-type="games" data-id="<?=$row["id"];?>">
                                                    <?php
                                                    echo $coment_array[1];
                                                    ?>
                                                </div>
                    ...
                    <div class="right-container floating-right">
                        ...
                        <div class="content-right">
                            <ul>
                                ...
                                <li>
                                    <label class="floating-left"><?=$Lang->Age;?> :</label><span title="<?=getProductAge($row["grade"]);?>"><?=getProductAge($row["grade"]);?> <?=$Lang->Year;?></span>
                                </li>
