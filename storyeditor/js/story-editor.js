...
function addPage(){
    var cpageid=$(".jq_questioni").length;
    $.ajax({
        method: "POST",
        url: "../platform/ajax/storyeditor.php?process=addpage",
        data: {storyid:window.storyid,storypath:window.storyPath},
        dataType: "json",
        success: function (data) {
            updateOrder();
            console.log(data);
            var html='<li class="jq_questioni ui-sortable-handle" pageid="'+data.page_id+'">'+
                ...
                '</label>'+
                '</li>';
            $("#questions_slider").append(html);
            $("#questions_slider .jq_questioni").last().click();
            setTimeout(function () {
                hideLoader();
                refreshElement();
                refreshPageSlider();
...
function fitPageSize(){
    // freeHeight=$(window).height()-$("header").height()-$("footer").height();
    freeHeight=785;
    p=freeHeight/$(".story-main-container").height();
    $(".story-main-container").height(freeHeight);
    $(".story-main-container").width($(".story-main-container").width()*p);
}
...
$(document).ready(function () {
    ...
    var evt         = new Event(),
        ...
        rg          = new RulersGuides(evt, dragdrop,document.getElementsByClassName('story-content-container'));
    ...
    $("#update_fontsize_widget").click(function () {
        ...
        $("#"+window.widgetID).attr("style",$("#"+window.widgetID).attr("style").toString()+";font-size:"+$("#font_size").val().toString()+"px !important");
        $("#"+window.widgetID).attr("style",$("#"+window.widgetID).attr("style").toString()+";line-height:"+$("#line-height").val().toString()+"px !important");
    });
    ...
    $("#save_setting").click(function () {
        ...
        showLoader();
        $("#quiz_top_title").html($("#general_title").val());
        $("#settings_form")[0].submit();
        if($("#Onepage").prop("checked")){
            if($("#width").val()==0){
                // $(".story-content-container").width($(".story-content-container").height()*2);
                $(".story-main-container").css({"width":"900px","height":"785px"});
            }else{
                // $(".story-content-container").width($(".story-content-container").height()*2);
                $(".story-main-container").css({"width":$("#width").val()+"px","height":$("#height").val()+"px"});
            }
            ...
            if($("#width").val()==0){
                // $(".story-content-container").width($(".story-content-container").height()*2);
                $(".story-main-container").css({"width":"450px","height":"785px"});
            }else{
                // $(".story-content-container").width($(".story-content-container").height()*2);
                p=785/$("#height").val();
                sheight=785;
                swidth=$("#width").val()*p;
                $(".story-main-container").css({"width":(swidth/2).toString()+"px","height":(sheight).toString()+"px"});
            }
        }else if($("#sliderview").prop("checked")){
            if($("#width").val()==0){
                // $(".story-content-container").width($(".story-content-container").height()*2);
                ...
                sheight=785;
                swidth=$("#width").val()*p;
                $(".story-main-container").css({"width":(swidth).toString()+"px","height":(sheight).toString()+"px"});
            }
    ...
    $("#questions_slider").sortable({
        ...
        update: function( event, ui ) {
            ...
            setTimeout(function(){
                ...
                $.ajax({
                    method: "POST",
                    url: "../platform/ajax/storyeditor.php?process=sortpages",
                    data: data,
                    dataType: "html",
                    dataType: "json",
                    success: function (result) {
                        ...
                        if(result.result==1){
                            ...
                            showMsg("error","unexpected error");
                        }
                    }
                });
            },100);
...
function refreshElement() {
    ...
    $(".resizable").resizable({
        start: function( event, ui ) {
            if ($(this).children(".real-content").children("iframe").length > 0) {
                iframelink = $(this).children(".real-content").children("iframe").attr("src");
                iframes='<iframe src="'+iframelink+'" frameborder="0" style="position:absolute;left: 0; top: 0;width: 100%;height: 100%;"></iframe>';
                imagelink='<img class="defult_after_iframe" src="'+SITE_URL+'storyeditor/thems/En/images/defaultdrag.svg" style="width:100%;height: 100%;">';
                $(this).children(".real-content").children("iframe").remove();
                $(this).children(".real-content").prepend(imagelink);
            }
        ...
        stop: function () {
            if ($(this).children(".real-content").children("iframe").length == 0) {
                $(this).children(".real-content").children(".defult_after_iframe").remove();
                $(this).children(".real-content").prepend(iframes);
            }

            $(this).outerWidth($(this).outerWidth()/$("#story-content-container").outerWidth()*100 +"%");
            $(this).outerHeight($(this).outerHeight()/$("#story-content-container").outerHeight()*100+"%");
    ...
    $(".cdraggable").draggable({
        ...
        stop: function () {
            // alert();
            var l = ( 100 * parseFloat($(this).css("left")) / parseFloat($(this).closest(".xdroppable").css("width")) ) + "%";
            $(this).css("left", l);
            //refreshElement();
        }
    });
    ...
    //$(".droppable").dropsetting_widgetpable({
    $("#story-content-container.droppable").droppable({
        drop: function (event, ui) {
            ...
            var offset=$(".story-content-container").offset();
            var left=((event.pageX-offset.left)/$(".story-content-container").width())*100;
            var top=((event.pageY-offset.top)/$(".story-content-container").height())*100;
            console.log("drag",event.pageX,offset.left,$(".story-content-container").width());
            var order=parseInt(window.order)+1;
            window.order=order;

            switch (itemTypeID) {
                case 'text':
                    $(this).append('<div id="'+newID+'" class="resizable draggable leyersA element" style="width: 230px; height: 100px; position: absolute; top: '+top+'%; left: '+left+'%; right: auto; bottom: auto;"><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="text"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="noclose dir_right floating-right flaticon-x"></div><div class="noclose fontsize_widget floating-right flaticon-x"></div> <div class="noclose animation_widget floating-right flaticon-x" style="right:101px"></div><div class="noclose animation_widget_new floating-right flaticon-x"></div><div class="noclose audio_sync floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><div class="real-content"><span class="poplinable" contenteditable="true">' + window.Lang.Text + '</span></div></div>');
                    document.execCommand('defaultParagraphSeparator', false, 'p');
                    $(".poplinable").popline({position: "fixed"});
                    break;
                case 'youtube':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:235px;height: 109px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="video"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content remove-margin"><iframe width="100%" height="100%" src="https://www.youtube.com/embed/sxUQsKxcOYM" frameborder="0" allowfullscreen=""></iframe></div></div>');
                    break;
                case 'sound':
                    // $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:303px;height:54px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="audio"></div><div class="noclose setting_widget floating-right flaticon-x" style="right: 50px;" widget_type="audio"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content"><audio controls=""><source src="sound.mp3" type="audio/mp3"></audio></div><div>');
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:350px;height:50px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="audio"></div><div class="noclose setting_widget floating-right flaticon-x" style="right: 50px;" widget_type="audio"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content"><div class="gallery type1"><div class="item-container"><a><div class="audio-player jq_player"><div id="play-btn" class="play-button"></div><div class="audio-wrapper" id="player-container" href="javascript:;"><audio id="player" ontimeupdate="initProgressBar(this)"><source src="sound.mp3" type="audio/mp3"></audio></div><div class="player-controls scrubber"><span id="seekObjContainer"><progress id="seekObj" class="seekObj" value="0" max="1" style=""></progress></span><br><small style="float: left; position: relative; left: 15px;color: #00ad68;font-size: 12px;display: none;" class="start-time"></small><small style="float: right; position: relative; right: 20px;color: #00ad68;font-size: 12px; display: none" class="end-time"></small></div><div class="album-image"></div></div></a></div></div></div><div>');

                    break;
                case 'video':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:303px;height:54px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="video"></div><div class="noclose setting_widget floating-right flaticon-x" style="right: 50px;" widget_type="video"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content"><video width="100%" height="100%" loop controls><source src="movie.ogg" type="video/ogg"></video></audio></div><div>');
                    break;
                case 'image':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element image" style="width:195px;height:141px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose flip_widget floating-right flaticon-x"></div><div class="noclose  move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="image"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="image" style="right: 50px"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content remove-margin"><img src="'+window.SITE_URL+'storyeditor/thems/En/images/1.svg" style="width:100%;height: 100%;"></div><div></div>');
                    break;
                case 'DoyouKnow':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:200px;height:100px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose  move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="DoyouKnow"></div><div class="noclose setting_widget floating-right flaticon-x" style="right: 50px;" widget_type="DoyouKnow"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content remove-margin"><img class="jq_douknow" src="'+window.SITE_URL+'storyeditor/thems/En/images/doyoujnow.svg" style="width:100%;height: 100%"></div><div></div>');
                    break;
                case 'Iframe':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:200px;height:100px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose  move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose edit_widget flaticon-pencil43" widget_type="Iframe"></div><div class="noclose setting_widget floating-right flaticon-x"  style="right: 50px" widget_type="Iframe"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content remove-margin"><iframe width="100%" height="100%" src="https://www.youtube.com/embed/KDmz6rZGDcI" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div><div></div>');
                    break;
                case 'LeftBallon':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element visible-element" style="width:230px;height:100px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x" style="right:101px"></div><div class="noclose audio_sync floating-right flaticon-x"></div><div class="noclose  move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="LeftBallon"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="noclose dir_right floating-right flaticon-x"></div><div class="noclose fontsize_widget floating-right flaticon-x"></div><div class="real-content remove-margin default-border"><div class="box1 sb7"><span class="poplinable ballon-text" contenteditable="true">' + window.Lang.Text + '</span></div></div><div></div>');
                    break;
                case 'RightBallon':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element visible-element" style="width:230px;height:100px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x" style="right:101px"></div><div class="noclose audio_sync floating-right flaticon-x"></div><div class="noclose  move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="RightBallon"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="noclose dir_right floating-right flaticon-x"></div><div class="noclose fontsize_widget floating-right flaticon-x"></div><div class="real-content remove-margin default-border"><div class="box1 sb8"><span class="poplinable ballon-text" contenteditable="true">' + window.Lang.Text + '</span></div></div><div></div>');
                    break;
                case 'Symbols':
                    $(this).append('<div id="'+newID+'" class="resizable draggable leyersA element" style="width: 200px; height: 100px; position: absolute; top: '+top+'%; left: '+left+'%; right: auto; bottom: auto;"><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" widget_type="text"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="noclose fontsize_widget floating-right flaticon-x"></div><div class="noclose Symbols_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><div class="real-content"><span class="poplinable symbol_font" style="font-size: 50px">0</span></div></div>');
                    document.execCommand('defaultParagraphSeparator', false, 'p');
                    $(".poplinable").popline({position: "fixed"});
                    $(this).find(".Symbols_widget ").click();
                    break;
                case 'Buttons_drag':
                    $(this).append('<div id="'+newID+'" class="resizable draggable leyersA element" style="width: 200px; height: 100px; position: absolute; top: '+top+'%; left: '+left+'%; right: auto; bottom: auto;"><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_buttons_widget floating-right flaticon-x" widget_type="text"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="noclose fontsize_widget floating-right flaticon-x"></div><div class="noclose buttons_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><div class="real-content"></div></div>');
                    document.execCommand('defaultParagraphSeparator', false, 'p');
                    $(".poplinable").popline({position: "fixed"});
                    $(this).find(".buttons_widget").click();
                    break;
                case 'record_drag':
                    $(this).append('<div id="' + newID + '" class="resizable draggable leyersA element" style="width:303px;height:54px;position: absolute;top: '+top+'%; left: '+left+'%;"><div class="noclose animation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose setting_widget floating-right flaticon-x" style="right: 50px;" widget_type="audio"></div><div class="noclose audio_widget floating-right flaticon-x"></div><div class="real-content"><audio class="plyer-control" controls><source src="sound.mp3" type="audio/mp3"></audio><a class="record-sound floating-left jq_recordsound"><span class="toolbar-button-state recording floating-left"></span><i class="floating-left"></i></a></div><div>');
                    break;
                case 'bganimation_drag':
                    $(this).append('<div id="'+newID+'" class="resizable draggable leyersA element" style="width: 200px; height: 100px; position: absolute; top: '+top+'%; left: '+left+'%; right: auto; bottom: auto;"><input type="number" class="noclose content-of-index" value="'+order+'"><div class="noclose delete_widget floating-right flaticon-x"></div><div class="noclose bganimation_widget floating-right flaticon-x"></div><div class="noclose move_handler flaticon-more9"></div><div class="real-content"><iframe src="'+window.SITE_URL+'storyeditor/BgAnimation/clouds/index.html" frameborder="0" style="position:absolute;left: 0; top: 0;width: 100%;height: 100%;"></iframe></div></div>');
                    document.execCommand('defaultParagraphSeparator', false, 'p');
                    $(".poplinable").popline({position: "fixed"});
                    $(this).find(".bganimation_widget").click();
                    break;
            }
            setTimeout(function () {
                refreshElement();
