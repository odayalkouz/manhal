...
//                            }
                            ...
                            $sql = "SELECT `books`.*,`categories`.*,`books`.`rating_count` as rate_count FROM `books` left OUTER JOIN `categories` ON `books`.`category`=`categories`.`catid` WHERE `books`.`status` =1 AND `books`.`store`=0 AND( `books`.`status`=1 " . $keyword_filter . $cat_filter .$grade_filter.$age_filter.$year_filter.$isbn_filter.$lang_filter.$price_filter.$semester_filter.$booktype_filter.$type_filter." ) "." ORDER BY `books`.`booktype` DESC,`books`.`name` DESC ".$pagination[0];
                            $result = $con->query($sql);
                            ?>
