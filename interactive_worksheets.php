...
$currentTab = "interactive-worksheet";
include "includes/function.php";
include_once "platform/includes/function.php";
include("includes/header.php");
?>
<!--<link rel="stylesheet" type="text/css" href="--><? //=SITE_URL;?><!--themes/main-Light-green---><? //=$session_lang;?><!--/css/media.css--><? //=$cash;?><!--">-->
                        ...
                        <div class="right-col-3 floating-left">
                            <div class="books-main-container-page clear-both">
                                ...
                                <div class="ui-panels-worksheet">
                                    ...
                                    $sql = "Select media.*,categories.name_ar, categories.name_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type=12 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter." ORDER BY `media`.`price` ASC, `media`.`id` DESC , `media`.`is_playlist` DESC";
                                    ...
                                    $url = "interactive-worksheets?";
                                    if (strpos($link, "?") === false) {
                                        $url = "interactive-worksheets?";
                                    } else {
                                        $arr = explode("?", $link);
                                        $getData = explode("&", $arr[1]);
                                        $url = "interactive-worksheets?" . $arr[1];
                                    }
                                    $pagination = getPagination($url, $num_rows);
                                    $sql = "Select media.*,categories.name_ar, categories.name_en From media Inner Join categories On media.category = categories.catid Where media.status = 1 And media.id > 0 And media.type=12 " . $keyword_filter . $cat_filter . $lang_filter . $age_filter . $grade_filter." ORDER BY `media`.`price` ASC,`media`.`id` DESC, `media`.`is_playlist` DESC " . $pagination[0];
                                    $result = $con->query($sql);
                                    while ($game = mysqli_fetch_assoc($result)) {
                                        echo paintmedia($game, 'interactive-worksheets');
                                    }
                                    ?>
                                    <div class="paging">
                                        <div class="content">
